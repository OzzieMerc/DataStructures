...
namespace UT
{
	void TestSorting()
	{
		Test("Sorting")
		{
			...
			Subtest("InsertionSort()")
			{
				int numbers[NUM_COUNT] = { 8, 6, 7, 5, 3, 0, 9, 4, 2, 1 };

				// Sort lowest to highest.
				Sort::InsertionSort<int>(numbers, NUM_COUNT, [](const int& _lhs, const int& _rhs) { return _lhs > _rhs; });
			...
			Subtest("InsertionSortAscending()") // Lowest to highest.
			{
				int numbers[NUM_COUNT] = { 8, 6, 7, 5, 3, 0, 9, 4, 2, 1 };

				Sort::InsertionSortAscending<int>(numbers, NUM_COUNT);

				for (unsigned int i = 1; i < NUM_COUNT; ++i)
					Require(numbers[i - 1] < numbers[i]);
			}

			Subtest("InsertionSortDescending()") // Highest to lowest.
			{
				int numbers[NUM_COUNT] = { 8, 6, 7, 5, 3, 0, 9, 4, 2, 1 };

				Sort::InsertionSortDescending<int>(numbers, NUM_COUNT);
